...
const MOLTBOOK_PATH = path_1.default.join(__dirname, '../moltbook.json');
const moltbookData = JSON.parse(fs_1.default.readFileSync(MOLTBOOK_PATH, 'utf-8'));
function searchMoltbook(query) {
...
function matchScore(contact, query) {
    ...
    if (name === q)
        return 100;
    if (contact.username?.toLowerCase() === q)
        return 90;
    if (name.startsWith(q))
        return 80;
    if (name.includes(q) || contact.username?.toLowerCase().includes(q))
        return 50;
    return 0;
}
...
async function searchAllTiers(telegramId, query) {
    ...
    if (isUsername) {
        ...
        const global = (await (0, db_1.searchRecipients)(query)) ?? [];
        const globalUsername = global.filter((c) => c.username?.toLowerCase() === query.slice(1).toLowerCase());
        if (globalUsername.length > 0)
            return [{ contact: globalUsername[0], tier: 2, tierLabel: 'global directory', score: 90 }];
...
function default_1(api) {
    ...
    api.registerTool({
        ...
        handler: async (input) => {
            ...
            // ── Phase 2: Execute ──────────────────────────────────────────────────
            if (confirmed === true) {
                const pending = await (0, db_1.getPendingPayment)(senderTelegramId);
                if (!pending) {
    ...
    api.registerTool({
        ...
        handler: async (input) => {
            await (0, db_1.clearPendingPayment)(input.senderTelegramId);
            return { status: 'cancelled', message: 'Payment cancelled.' };
        },
    });
}
