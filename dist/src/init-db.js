...
async function main() {
    ...
    for (const table of ['tx_history', 'pending_payments', 'recipients', 'users']) {
        ...
        if (error) {
            ...
            if (e2)
                console.log(`  ⚠  clear ${table}: ${e2.message}`);
            else
    ...
    for (const biz of GLOBAL_DIRECTORY) {
        ...
        if (error) {
            if (error.message.includes('username')) {
                ...
                if (e2)
                    console.log(`  ✗  ${biz.name}: ${e2.message}`);
                else
                    console.log(`  ✓  ${biz.name} (no username)`);
            }
            else {
    ...
    if (re)
        console.log('  ✗  recipients verify:', re.message);
    else
        console.log(`  ✓  recipients: ${recs.length} rows`);
