...
async function test() {
  ...
  const matches = await searchRecipients('coffee');
  const coffee = matches[0];
  console.log('Sending to:', coffee.name, coffee.wallet_address);

  console.log('Balance before:', await getBalance(senderAddress), 'USDC');

  const txHash = await transfer(encryptedKey, coffee.wallet_address, 0.1);
  console.log('✅ TX Hash:', txHash);

  // Check receiver got it
  console.log('Coffee balance after:', await getBalance(coffee.wallet_address), 'USDC');
}
